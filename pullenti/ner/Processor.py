...
class Processor(object):
    ...
    def findAnalyzer(self, name : str) -> 'Analyzer':
        ...
        inoutres2667 = Utils.tryGetValue(self.__m_analyzers_hash, Utils.ifNotNull(name, ""), wrapa2666)
        a = wrapa2666.value
        if (not inoutres2667): 
            return None
        else: 
    ...
    def processNext(self, ar : 'AnalysisResult') -> None:
        ...
        if (ar is None): 
            return
        kit = AnalysisKit._new2668(self, ar.ontology)
        kit._initFrom(ar)
    ...
    def _process(self, text : 'SourceOfAnalysis', no_entities_regine : bool, no_log : bool, ext_ontology : 'ExtOntology'=None, lang : 'MorphLang'=None) -> 'AnalysisResult':
        ...
        kit = AnalysisKit._new2669(text, False, lang, self.__progress_changed_event_handler_on_progress_handler, ext_ontology, self)
        ar = AnalysisResult()
        sw0.stop()
    ...
    def __prepareProgress(self) -> None:
        with self.__m_progress_peaces_lock: 
            ...
            max0_ = co * 100
            max0_ /= (total)
            self.__m_progress_peaces[self] = ProgressPeace._new2670(0, max0_)
            for wf in self.analyzers: 
                min0_ = max0_
                ...
                if (not wf in self.__m_progress_peaces): 
                    self.__m_progress_peaces[wf] = ProgressPeace._new2670(min0_, max0_)
    ...
    def _onProgressHandler(self, sender : object, e0_ : ProgressEventArgs) -> None:
        ...
        if (e0_.progressPercentage >= 0): 
            with self.__m_progress_peaces_lock: 
                wrappi2672 = RefOutArgWrapper(None)
                inoutres2673 = Utils.tryGetValue(self.__m_progress_peaces, Utils.ifNotNull(sender, self), wrappi2672)
                pi0_ = wrappi2672.value
                if (inoutres2673): 
    ...
    def manageReferentLinks(self) -> None:
        if (self.__m_refs is not None): 
            for pr in self.__m_refs: 
                ...
                inoutres2677 = Utils.tryGetValue(self.__m_links2, pr.identity, wrapr2676)
                r = wrapr2676.value
                if (pr.identity is not None and self.__m_links2 is not None and inoutres2677): 
                    pr.owner_referent.uploadSlot(pr.owner_slot, r)
                else: 
                    wrapr2674 = RefOutArgWrapper(None)
                    inoutres2675 = Utils.tryGetValue(self.__m_links, pr.value, wrapr2674)
                    r = wrapr2674.value
        ...
        try: 
            ...
            for x in xml0_: 
                ...
                if (att is not None and att[1] == "true"): 
                    pr = ProxyReferent._new2678(Utils.getXmlInnerText(x), res)
                ...
                if ((att) is not None): 
                    wrapcou2679 = RefOutArgWrapper(0)
                    inoutres2680 = Utils.tryParseInt(att[1], wrapcou2679)
                    cou = wrapcou2679.value
                    if (inoutres2680): 
                        slot.count = cou
