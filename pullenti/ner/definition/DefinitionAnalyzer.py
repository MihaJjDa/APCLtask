...
class DefinitionAnalyzer(Analyzer):
        ...
        if (kit.ontology is not None): 
            ...
            for it in kit.ontology.items: 
                if (isinstance(it.referent, DefinitionReferent)): 
                    ...
                    if (not termin in oh): 
                        oh[termin] = True
                        onto.add(Termin._new1087(termin, termin))
    ...
    def __tryAttachEnd(self, t : 'Token', onto : 'TerminCollection', max_char : int) -> typing.List['ReferentToken']:
        ...
        if ((((l1.end_char - l0.end_char)) * 2) > ((r1.end_char - r0.end_char))): 
            return None
        dr = DefinitionReferent._new1088(DefinitionKind.DEFINITION)
    ...
    def __tryAttachMiscToken(t : 'Token') -> 'MetaToken':
        ...
        if (t.isValue("КАК", None)): 
            ...
            while True:
                ...
                break
            if (t1 is not None): 
                res = MetaToken._new809(t, t1, MiscHelper.getTextValue(t, t1, GetTextAttr.KEEPQUOTES))
        ...
        if (npt is not None): 
            if (DefinitionAnalyzer.__m_misc_first_words.tryParse(npt.noun.begin_token, TerminParseAttr.NO) is not None): 
                res = MetaToken._new809(t, npt.end_token, npt.getNormalCaseText(None, True, MorphGender.UNDEFINED, False))
        ...
        if (t.isValue("В", None)): 
            ...
            if (npt is not None): 
                if (npt.noun.isValue("СМЫСЛ", None)): 
                    res = MetaToken._new809(t, npt.end_token, MiscHelper.getTextValue(t, npt.end_token, GetTextAttr.NO))
                    res.morph.class0_ = MorphClass.NOUN
                    return res
        return None
