...
class DecreeChangeToken(MetaToken):
    ...
    def tryAttach(t : 'Token', main : 'DecreeChangeReferent'=None, ignore_newlines : bool=False, change_stack : typing.List['Referent']=None, is_in_edition : bool=False) -> 'DecreeChangeToken':
        ...
        if (((isinstance(tt, TextToken)) and t.is_newline_before and not ignore_newlines) and tt.isValue("ВНЕСТИ", "УНЕСТИ") and ((((tt.next0_ is not None and tt.next0_.isValue("В", "ДО"))) or (tt).term == "ВНЕСТИ" or (tt).term == "УНЕСТИ"))): 
            res = DecreeChangeToken._new766(tt, tt, DecreeChangeTokenTyp.STARTMULTU)
        ...
        if (((not ignore_newlines and t.is_newline_before and ((tt.isValue("ПРИЗНАТЬ", "ВИЗНАТИ") or tt.isValue("СЧИТАТЬ", "ВВАЖАТИ")))) and tt.next0_ is not None and tt.next0_.isValue("УТРАТИТЬ", "ВТРАТИТИ")) and tt.next0_.next0_ is not None and tt.next0_.next0_.isValue("СИЛА", "ЧИННІСТЬ")): 
            res = DecreeChangeToken._new767(tt, tt.next0_.next0_, DecreeChangeTokenTyp.ACTION, DecreeChangeKind.EXPIRE)
        ...
        if ((not ignore_newlines and ((t.is_newline_before or tt == t)) and tt.isValue("УТРАТИТЬ", "ВТРАТИТИ")) and tt.next0_ is not None and tt.next0_.isValue("СИЛА", "ЧИННІСТЬ")): 
            res = DecreeChangeToken._new766(tt, tt.next0_, DecreeChangeTokenTyp.UNDEFINED)
            tt = tt.next0_
        ...
        if (not ignore_newlines and t.is_newline_before): 
            if (tt.isValue("СЛОВО", None)): 
                pass
            res = DecreeChangeToken._new766(tt, tt, DecreeChangeTokenTyp.STARTSINGLE)
            first_pass2827 = True
            while True:
                ...
                if (li is None and tt.isValue("ПРИМЕЧАНИЕ", "ПРИМІТКА")): 
                    li = list()
                    li.append(PartToken._new770(tt, tt, PartToken.ItemType.NOTICE))
                ...
                if (tt == res.begin_token and main is not None): 
                    ...
                    if (npt is not None): 
                        ...
                        if ((tt1 is not None and tt1.isValue("ИЗЛОЖИТЬ", "ВИКЛАСТИ") and tt1.next0_ is not None) and tt1.next0_.isValue("В", None)): 
                            pt = PartToken._new770(tt, npt.end_token, PartToken.ItemType.APPENDIX)
        ...
        if (tok is not None): 
            if (isinstance(tok.termin.tag, DecreeChangeKind)): 
                res = DecreeChangeToken._new767(tt, tok.end_token, DecreeChangeTokenTyp.ACTION, Utils.valToEnum(tok.termin.tag, DecreeChangeKind))
            ...
            if (isinstance(tok.termin.tag, DecreeChangeValueKind)): 
                res = DecreeChangeToken._new766(tt, tok.end_token, DecreeChangeTokenTyp.VALUE)
                res.change_val = DecreeChangeValueReferent()
        ...
        if (((is_nex_change > 0 and BracketHelper.isBracket(t, True))) or ((is_nex_change > 1 and t.isValue("ПРИЛОЖЕНИЕ", "ДОДАТОК")))): 
            res = DecreeChangeToken._new766(t, t, DecreeChangeTokenTyp.VALUE)
            res.change_val = DecreeChangeValueReferent._new775(DecreeChangeValueKind.TEXT)
            if (is_in_edition): 
                return res
    ...
    def __tryAttachList(t : 'Token') -> typing.List['DecreeChangeToken']:
        ...
        while True:
            ...
            if (d is None): 
                ...
                if (pts is not None): 
                    d = DecreeChangeToken._new776(pts[0].begin_token, pts[len(pts) - 1].end_token, DecreeChangeTokenTyp.UNDEFINED, pts)
                else: 
    ...
    def _initialize() -> None:
        ...
        t = Termin._new117("ИЗЛОЖИТЬ В СЛЕДУЮЩЕЙ РЕДАКЦИИ", DecreeChangeKind.NEW)
        ...
        t = Termin._new455("ВИКЛАСТИ В НАСТУПНІЙ РЕДАКЦІЇ", MorphLang.UA, DecreeChangeKind.NEW)
        ...
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ПРИЗНАТЬ УТРАТИВШИМ СИЛУ", DecreeChangeKind.EXPIRE)
        t.addVariant("СЧИТАТЬ УТРАТИВШИМ СИЛУ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ВИЗНАТИ таким, що ВТРАТИВ ЧИННІСТЬ", MorphLang.UA, DecreeChangeKind.EXPIRE)
        t.addVariant("ВВАЖАТИ таким, що ВТРАТИВ ЧИННІСТЬ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ИСКЛЮЧИТЬ", DecreeChangeKind.REMOVE)
        t.addVariant("ИСКЛЮЧИВ ИЗ НЕГО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ВИКЛЮЧИТИ", MorphLang.UA, DecreeChangeKind.REMOVE)
        t.addVariant("ВИКЛЮЧИВШИ З НЬОГО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("СЧИТАТЬ", DecreeChangeKind.CONSIDER)
        t.addVariant("СЧИТАТЬ СООТВЕТСТВЕННО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ВВАЖАТИ", MorphLang.UA, DecreeChangeKind.CONSIDER)
        t.addVariant("ВВАЖАТИ ВІДПОВІДНО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ЗАМЕНИТЬ", DecreeChangeKind.EXCHANGE)
        t.addVariant("ЗАМЕНИВ В НЕМ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ЗАМІНИТИ", MorphLang.UA, DecreeChangeKind.EXCHANGE)
        t.addVariant("ЗАМІНИВШИ В НЬОМУ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ДОПОЛНИТЬ", DecreeChangeKind.APPEND)
        t.addVariant("ДОПОЛНИВ ЕГО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ДОПОВНИТИ", MorphLang.UA, DecreeChangeKind.APPEND)
        t.addVariant("ДОПОВНИВШИ ЙОГО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("СЛОВО", DecreeChangeValueKind.WORDS)
        t.addVariant("АББРЕВИАТУРА", False)
        t.addVariant("АБРЕВІАТУРА", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ЦИФРА", DecreeChangeValueKind.NUMBERS)
        t.addVariant("ЧИСЛО", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("ПРЕДЛОЖЕНИЕ", DecreeChangeValueKind.SEQUENCE)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ПРОПОЗИЦІЯ", MorphLang.UA, DecreeChangeValueKind.SEQUENCE)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("СНОСКА", DecreeChangeValueKind.FOOTNOTE)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("ВИНОСКА", MorphLang.UA, DecreeChangeValueKind.FOOTNOTE)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("БЛОК", DecreeChangeValueKind.BLOCK)
        t.addVariant("БЛОК СО СЛОВАМИ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("БЛОК", MorphLang.UA, DecreeChangeValueKind.BLOCK)
        t.addVariant("БЛОК ЗІ СЛОВАМИ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new117("В СООТВЕТСТВУЮЩИХ ЧИСЛЕ И ПАДЕЖЕ", DecreeChangeValueKind.ROBUSTWORDS)
        t.addVariant("В СООТВЕТСТВУЮЩЕМ ПАДЕЖЕ", False)
        t.addVariant("В СООТВЕТСТВУЮЩЕМ ЧИСЛЕ", False)
        DecreeChangeToken.__m_terms.add(t)
        t = Termin._new455("У ВІДПОВІДНОМУ ЧИСЛІ ТА ВІДМІНКУ", MorphLang.UA, DecreeChangeValueKind.ROBUSTWORDS)
                    ...
                    for np in tok.new_parts: 
                        ...
                        if (len(np.values) == 0): 
                            if (eq_lev_val is None): 
                                ...
                                wrapn799 = RefOutArgWrapper(0)
                                inoutres800 = Utils.tryParseInt(eq_lev_val, wrapn799)
                                n = wrapn799.value
                                if (inoutres800): 
    ...
    @staticmethod
    def _new766(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeChangeTokenTyp') -> 'DecreeChangeToken':
        res = DecreeChangeToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new767(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeChangeTokenTyp', _arg4 : 'DecreeChangeKind') -> 'DecreeChangeToken':
        res = DecreeChangeToken(_arg1, _arg2)
        res.typ = _arg3
        res.act_kind = _arg4
        return res
    ...
    @staticmethod
    def _new776(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeChangeTokenTyp', _arg4 : typing.List['PartToken']) -> 'DecreeChangeToken':
        res = DecreeChangeToken(_arg1, _arg2)
        res.typ = _arg3
        res.parts = _arg4
        return res