...
class DecreeAnalyzer(Analyzer):
    ...
    class ThisDecree(MetaToken):
        ...
        def tryAttachBack(t : 'Token', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (ukaz is not None): 
                if (base_typ is not None and base_typ.value is not None and (("ДОГОВОР" in base_typ.value or "ДОГОВІР" in base_typ.value))): 
                    return DecreeAnalyzer.ThisDecree._new1080(ukaz, ukaz, True, base_typ.value)
            return None
        ...
        def tryAttach(dtok : 'PartToken', base_typ : 'DecreeToken') -> 'ThisDecree':
            ...
            if (not ((isinstance(tt, TextToken)))): 
                if ((isinstance(tt, ReferentToken)) and (isinstance(tt.getReferent(), DecreeReferent))): 
                    return DecreeAnalyzer.ThisDecree._new1081(t, tt, Utils.asObjectOrNull(tt.getReferent(), DecreeReferent))
                return None
        ...
        @staticmethod
        def _new1080(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool, _arg4 : str) -> 'ThisDecree':
        ...
        @staticmethod
        def _new1081(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DecreeReferent') -> 'ThisDecree':
            res = DecreeAnalyzer.ThisDecree(_arg1, _arg2)
            res.real = _arg3
            return res
    ...
    def __TryAttach(dts : typing.List['DecreeToken'], base_typ : 'DecreeToken', after_decree : bool, ad : 'AnalyzerData') -> typing.List['ReferentToken']:
        ...
        rt22 = DecreeAnalyzer._tryAttachApproved(rt.end_token.next0_, ad, True)
        if (rt22 is not None): 
            ...
            if (dr00.typ is None): 
                for s in dr00.slots: 
                    if (s.type_name == DecreeReferent.ATTR_DATE or s.type_name == DecreeReferent.ATTR_SOURCE): 
                        if (dec.findSlot(s.type_name, None, True) is None): 
                            dec.addSlot(s.type_name, s.value, False, 0)
                dr00 = (None)
            if (dr00 is not None): 
                rt_li.append(rt22)
        ...
        if ((len(dts) == 2 and dts[0].typ == DecreeToken.ItemType.TYP and dts[0].typ_kind == DecreeKind.STANDARD) and dts[1].typ == DecreeToken.ItemType.NUMBER): 
            ...
            while ttt is not None: 
                ...
                if (nu is None or nu.typ != DecreeToken.ItemType.NUMBER): 
                    break
                dr1 = DecreeReferent._new1068(dec.typ)
                dr1._addNumber(nu)
        ...
        while True:
            ...
            if (tok is not None and tok.begin_token == tok.end_token and tok.chars.is_all_lower): 
                ...
                if (not ok): 
                    tok = (None)
            if (tok is not None): 
                rt0 = DecreeAnalyzer._tryAttachApproved0(t, ad)
                if (rt0 is not None): 
                    tok = (None)
            if (tok is not None): 
            ...
            if (rtli is None or ((len(rtli) == 1 and (len(dts) < 3) and dts[0].value == "РЕГЛАМЕНТ"))): 
                rt = DecreeAnalyzer._tryAttachApproved0(t, ad)
                if (rt is not None): 
                    rtli = list()
                    rtli.append(rt)
            if (rtli is not None): 
            ...
            if (dcht is not None and has_start_change > 0): 
                ...
                if (dcht.typ == DecreeChangeTokenTyp.STARTMULTU): 
                    ...
                    if (len(change_stack) > 0 or dcht.decree is not None): 
                        root_change = (Utils.asObjectOrNull(ad.registerReferent(DecreeChangeReferent._new1069(DecreeChangeKind.CONTAINER)), DecreeChangeReferent))
    ...
    @staticmethod
    def _tryAttachApproved0(t : 'Token', ad : 'AnalyzerData') -> 'ReferentToken':
        from pullenti.ner.decree.internal.DecreeToken import DecreeToken
        from pullenti.ner.decree.DecreeReferent import DecreeReferent
        if (t is None): 
            return None
        ...
        if (br is not None and br.length_char > 20): 
            rt0 = DecreeAnalyzer._tryAttachApproved(br.end_token.next0_, ad, False)
            if (rt0 is not None): 
        ...
        while True:
            ...
            if (DecreeToken.isKeyword(tt, False) is not None and tt.chars.is_capital_upper): 
                break
            rt0 = DecreeAnalyzer._tryAttachApproved(tt, ad, True)
            if (rt0 is not None): 
                t1 = tt.previous
    ...
    @staticmethod
    def _tryAttachApproved(t : 'Token', ad : 'AnalyzerData', must_be_comma : bool=True) -> 'ReferentToken':
        from pullenti.ner.decree.internal.DecreeToken import DecreeToken
        from pullenti.ner.decree.DecreeReferent import DecreeReferent
        ...
        olev = None
        lev = 0
        wrapolev1073 = RefOutArgWrapper(None)
        inoutres1074 = Utils.tryGetValue(kit.misc_data, "dovr", wrapolev1073)
    ...
    def __tryAttachPulishers(self, dts : typing.List['DecreeToken']) -> typing.List['ReferentToken']:
        ...
        while True:
            ...
            if (pt is not None): 
                ...
                if (pub_part is None): 
                    pub_part = DecreePartReferent._new1075(pub)
            ...
            if (isinstance(t, NumberToken)): 
                ...
                if (rt is not None): 
                    date_ = DecreeToken._new815(rt.begin_token, rt.end_token, DecreeToken.ItemType.DATE)
                    date_.ref = rt
                    pub0 = (None)
                    pub_part0 = (None)
                    if (t0 is None): 
        ...
        while i < len(res): 
            ...
            while et.begin_char > res[i].begin_char:
                if (et.isChar(',') or et.morph.class0_.is_conjunction or et.is_hiphen): 
                    ...
                    break
            res1.append(MetaToken._new809(res[i].begin_token, et, li))
            i = (j - 1)
            i += 1
        return res1