...
class MeasureToken(MetaToken):
    ...
    def tryParseMinimal(t : 'Token', add_units : 'TerminCollection', can_omit_number : bool=False) -> 'MeasureToken':
        ...
        if ((len(mt) == 1 and len(mt[0].units) == 1 and mt[0].units[0].is_doubt) and not mt[0].is_newline_before): 
            return None
        if (len(mt) == 1): 
            res = MeasureToken._new1506(mt[0].begin_token, mt[len(mt) - 1].end_token, mt[0])
            res.__parseInternals(add_units)
            return res
        res = MeasureToken(mt[0].begin_token, mt[len(mt) - 1].end_token)
        for m in mt: 
            res.internals.append(MeasureToken._new1506(m.begin_token, m.end_token, m))
        return res
    ...
    def __parseInternals(self, add_units : 'TerminCollection') -> None:
        if (self.end_token.next0_ is not None and ((self.end_token.next0_.isCharOf("\\/") or self.end_token.next0_.isValue("ПРИ", None)))): 
            mt1 = MeasureToken.tryParse(self.end_token.next0_.next0_, add_units, True, False)
            if (mt1 is not None): 
                ...
                if (mt is not None and len(mt.units) > 0 and not UnitToken.canBeEquals(self.nums.units, mt.units)): 
                    self.internals.append(MeasureToken._new1506(mt.begin_token, mt.end_token, mt))
                    self.end_token = mt.end_token
    
    @staticmethod
    def tryParse(t : 'Token', add_units : 'TerminCollection', can_be_set : bool=True, can_units_absent : bool=False) -> 'MeasureToken':
        ...
        if (tt is not None): 
            t = tt.next0_
        npt = NounPhraseHelper.tryParse(t, Utils.valToEnum((NounPhraseParseAttr.PARSEPREPOSITION) | (NounPhraseParseAttr.IGNOREBRACKETS), NounPhraseParseAttr), 0)
        if (npt is None): 
        ...
        while True:
            ...
            if ((isinstance(tt, NumberToken)) and (tt).typ == NumberSpellingType.WORDS): 
                npt3 = NounPhraseHelper.tryParse(tt, NounPhraseParseAttr.PARSENUMERICASADJECTIVE, 0)
                if (npt3 is not None): 
                    tt = npt3.end_token
                    t1 = tt
                    if (len(internals_) == 0): 
            ...
            if (((tt.is_comma or tt.isChar('('))) and tt.next0_ is not None): 
                ...
                if (uu is not None): 
                    ...
                    if (tt.isChar('(') and t1.next0_ is not None and t1.next0_.isChar(')')): 
                        ...
                        uu = UnitToken.tryParseList(t1.next0_.next0_, add_units, False)
            ...
            if (tt.isValue("ЯМЗ", None)): 
                pass
            npt2 = NounPhraseHelper.tryParse(tt, Utils.valToEnum((NounPhraseParseAttr.PARSEPREPOSITION) | (NounPhraseParseAttr.IGNOREBRACKETS), NounPhraseParseAttr), 0)
            if (npt2 is None): 
        ...
        if (len(mts) > 1 and len(internals_) == 0): 
            if (len(mt.units) == 0): 
                if (units is not None): 
                    for m in mts: 
                        m.units = units
            res1 = MeasureToken._new1511(t0, mts[len(mts) - 1].end_token, name_.morph, True)
            res1.name = MiscHelper.getTextValueOfMetaToken(name_, GetTextAttr.FIRSTNOUNGROUPTONOMINATIVE)
            k = 0
            while k < len(mts): 
                ttt = MeasureToken._new1506(mts[k].begin_token, mts[k].end_token, mts[k])
                if (whd is not None): 
        ...
        if (not mt.is_whitespace_before): 
            if (mt.begin_token.previous is None): 
                return None
            if (mt.begin_token.previous.isCharOf(":),") or mt.begin_token.previous.is_table_control_char): 
                pass
            else: 
                return None
        ...
        if (minmax > 0 and mt.single_val is not None): 
            ...
            mt.single_val = (None)
        if (len(mt.units) == 0): 
            units = UnitToken.tryParseList(mt.end_token.next0_, add_units, True)
            if (units is None): 
                if (can_units_absent): 
                    pass
                else: 
        ...
        if ((mts1 is not None and len(mts1) == 1 and (t1.whitespaces_before_count < 3)) and len(mts1[0].units) > 0 and not UnitToken.canBeEquals(mts[0].units, mts1[0].units)): 
            res.is_set = True
            res.nums = (None)
            res.internals.append(MeasureToken._new1506(mt.begin_token, mt.end_token, mt))
            res.internals.append(MeasureToken._new1506(mts1[0].begin_token, mts1[0].end_token, mts1[0]))
            res.end_token = mts1[0].end_token
        return res
    
    @staticmethod
    def _new1506(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'NumbersWithUnitToken') -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.nums = _arg3
        return res
    
    @staticmethod
    def _new1511(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : bool) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.reliable = _arg4
        return res
    ...
    @staticmethod
    def _new1513(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'MorphCollection', _arg4 : typing.List['MeasureToken']) -> 'MeasureToken':
        res = MeasureToken(_arg1, _arg2)
        res.morph = _arg3
        res.internals = _arg4
        return res