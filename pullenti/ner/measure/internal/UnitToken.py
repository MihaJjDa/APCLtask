...
class UnitToken(MetaToken):
    ...
    @staticmethod
    def tryParseList(t : 'Token', add_units : 'TerminCollection', parse_unknown_units : bool=False) -> typing.List['UnitToken']:
        ut = UnitToken.tryParse(t, add_units, None, parse_unknown_units)
        if (ut is None): 
            return None
        res = list()
        res.append(ut)
        ...
        while tt is not None: 
            ut = UnitToken.tryParse(tt, add_units, res[len(res) - 1], True)
            if (ut is None): 
                break
            res.append(ut)
            tt = ut.end_token
    ...
    def __correct(self) -> None:
        ...
        if (t.isChar('³')): 
            ...
            num = (t).int_value
        elif ((t.isChar('<') and (isinstance(t.next0_, NumberToken)) and (t.next0_).int_value is not None) and t.next0_.next0_ is not None and t.next0_.next0_.isChar('>')): 
    ...
    @staticmethod
    def _new1621(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'UnitReferent') -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.ext_onto = _arg3
        return res
    
    @staticmethod
    def _new1622(_arg1 : 'Token', _arg2 : 'Token', _arg3 : int, _arg4 : bool) -> 'UnitToken':
        res = UnitToken(_arg1, _arg2)
        res.pow0_ = _arg3
        res.is_doubt = _arg4
        return res