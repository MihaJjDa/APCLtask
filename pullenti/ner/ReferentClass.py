...
class ReferentClass:
    ...
    def addFeature(self, attr_name : str, attr_caption : str, low_bound : int=0, up_bound : int=0) -> 'Feature':
        ...
        """
        res = Feature._new2692(attr_name, attr_caption, low_bound, up_bound)
    ...
    def findFeature(self, name_ : str) -> 'Feature':
        ...
        """
        wrapres2693 = RefOutArgWrapper(None)
        inoutres2694 = Utils.tryGetValue(self.__m_attrs, name_, wrapres2693)
        res = wrapres2693.value
        if (not inoutres2694): 
            return None
        else: 
            return res
