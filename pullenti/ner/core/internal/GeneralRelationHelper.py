...
class GeneralRelationHelper:
    ...
    class Node:
        ...
        @staticmethod
        def _new462(_arg1 : 'Referent', _arg2 : 'AnalyzerData') -> 'Node':
    ...
    def refreshGenerals(proc : 'Processor', kit : 'AnalysisKit') -> None:
        ...
        for a in proc.analyzers: 
            ...
            for r in ad.referents: 
                nod = GeneralRelationHelper.Node._new462(r, ad)
                all_refs.append(nod)
                r.tag = nod
                wrapsi465 = RefOutArgWrapper(None)
                inoutres466 = Utils.tryGetValue(all0_, a.name, wrapsi465)
                si = wrapsi465.value
                if (not inoutres466): 
                    si = dict()
                    all0_[a.name] = si
                ...
                for s in strs: 
                    li = [ ]
                    wrapli463 = RefOutArgWrapper(None)
                    inoutres464 = Utils.tryGetValue(si, s, wrapli463)
                    li = wrapli463.value
                    if (not inoutres464): 
                        li = list()
                        si[s] = li
