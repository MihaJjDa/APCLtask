...
class NounPhraseItem(MetaToken):
        ...
        for it in self.morph.items: 
            ...
            if (single_number and v.single_number_value is not None): 
                if (mc is not None and ((gender == MorphGender.NEUTER or gender == MorphGender.FEMINIE)) and mc.is_adjective): 
                    bi = MorphBaseInfo._new467(MorphClass(mc), gender, MorphNumber.SINGULAR, MorphCase.NOMINATIVE, self.morph.language)
            ...
            if (str.isdigit(v.normal_value[0]) and mc is not None and mc.is_adjective): 
                ...
                if (inoutres469): 
                    str0_ = NumberHelper.getNumberAdjective(val, gender, (MorphNumber.SINGULAR if single_number or val == 1 else MorphNumber.PLURAL))
                    if (str0_ is not None): 
                        res = str0_
                        if (v.undef_coef == 0): 
                            break
    ...
    @staticmethod
    def _new470(_arg1 : 'Token', _arg2 : 'Token', _arg3 : bool) -> 'NounPhraseItem':
