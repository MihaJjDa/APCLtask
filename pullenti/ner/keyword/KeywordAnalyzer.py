...
class KeywordAnalyzer(Analyzer):
    ...
    def process(self, kit : 'AnalysisKit') -> None:
        ...
        while True:
            ...
            if (npt is None): 
                ...
                if (mc.is_verb and not mc.is_preposition): 
                    ...
                    if (t.isValue("МОЧЬ", None) or t.isValue("WOULD", None)): 
                        continue
                    kref = KeywordReferent._new1489(KeywordType.PREDICATE)
                    ...
                    rt1 = ReferentToken._new719(ad.registerReferent(kref), t, t, t.morph)
                    kit.embedToken(rt1)
                    t = (rt1)
                    continue
                continue
            ...
            while True:
                ...
                if (mc.is_misc): 
                    if (MiscHelper.isEngArticle(tt)): 
                        continue
                kref = KeywordReferent._new1489(KeywordType.OBJECT)
                norm = (tt).getLemma()
                kref.addSlot(KeywordReferent.ATTR_VALUE, norm, False, 0)
                if (norm != "ЕСТЬ"): 
                ...
                kref = (Utils.asObjectOrNull(ad.registerReferent(kref), KeywordReferent))
                KeywordAnalyzer.__setRank(kref, cur, max0_)
                rt1 = ReferentToken._new719(kref, tt, tt, tt.morph)
                kit.embedToken(rt1)
            ...
            if (len(li) > 1): 
                ...
                kref = (Utils.asObjectOrNull(ad.registerReferent(kref), KeywordReferent))
                KeywordAnalyzer.__setRank(kref, cur, max0_)
                rt1 = ReferentToken._new719(kref, t0, t, npt.morph)
                kit.embedToken(rt1)
                t = (rt1)
        cur = 0
        t = kit.first_token
        ...
        while True:
            ...
            rt1 = ReferentToken._new719(kw_un, t, t1, t.morph)
            kit.embedToken(rt1)
            t = (rt1)
        ...
        if (self.annotation_max_sentences > 0): 
            ano = AutoannoSentToken.createAnnotation(kit, self.annotation_max_sentences)
            if (ano is not None): 
    ...
    def __addReferents(self, ad : 'AnalyzerData', t : 'Token', cur : int, max0_ : int) -> 'Token':
        ...
        if (isinstance(r, DenominationReferent)): 
            ...
            kref0 = KeywordReferent._new1489(KeywordType.REFERENT)
            for s in dr.slots: 
        ...
        if (isinstance(r, MoneyReferent)): 
            ...
            kref0 = KeywordReferent._new1489(KeywordType.OBJECT)
        ...
        while tt is not None and tt.end_char <= t.end_char: 
            ...
            tt = tt.next0_
        kref = KeywordReferent._new1489(KeywordType.REFERENT)
        norm = None
        if (r.type_name == "GEO"): 
