...
class InstrToken(MetaToken):
    ...
    def initialize() -> None:
        ...
        InstrToken.__m_ontology.add(t)
        t = Termin._new112("ФАМИЛИЯ ИМЯ ОТЧЕСТВО", "ФИО")
        t.addAbridge("Ф.И.О.")
        InstrToken.__m_ontology.add(t)
        t = Termin._new1404("ПРІЗВИЩЕ ІМЯ ПО БАТЬКОВІ", MorphLang.UA, "ФИО")
        InstrToken.__m_ontology.add(t)
        t = Termin("ФАМИЛИЯ")
        t.addAbridge("ФАМ.")
        InstrToken.__m_ontology.add(t)
        ...
        while True:
            ...
            if (is_start_of_line and t == t0): 
                ...
                if ((isinstance(t.getReferent(), PersonReferent)) or (isinstance(t.getReferent(), PersonPropertyReferent)) or (isinstance(t.getReferent(), InstrumentParticipant))): 
                    return InstrToken.__correctPerson(InstrToken._new1405(t00, t, ILTypes.PERSON, t))
                is_ref = False
                if (isinstance(t.getReferent(), PersonPropertyReferent)): 
                    ...
                    if (rt is not None): 
                        if (isinstance(rt.referent, PersonReferent)): 
                            return InstrToken._new1406(t00, rt.end_token, ILTypes.PERSON)
                ...
                if (tt is None and is_ref): 
                    return InstrToken._new1405(t00, Utils.ifNotNull(t11, t), ILTypes.PERSON, t)
            ...
            if (dt is not None): 
                if (dt.typ == DecreeToken.ItemType.TYP and not t.chars.is_all_lower): 
                    ...
                    while tt is not None: 
                        ...
                        tt = tt.next0_
                    if (not has_verb_): 
                        res2 = InstrToken._new1409(t0, dt.end_token, ILTypes.TYP, Utils.ifNotNull(dt.full_value, dt.value))
                ...
                if (dt.typ == DecreeToken.ItemType.NUMBER): 
                    if (t != t0): 
                        break
                    return InstrToken._new1409(t0, dt.end_token, ILTypes.REGNUMBER, dt.value)
                if (dt.typ == DecreeToken.ItemType.ORG): 
                    if (t != t0): 
                        break
                    return InstrToken._new1411(t0, dt.end_token, ILTypes.ORGANIZATION, dt.ref, dt.value)
                if (dt.typ == DecreeToken.ItemType.TERR): 
                    if (t != t0): 
                        break
                    re = InstrToken._new1411(t0, dt.end_token, ILTypes.GEO, dt.ref, dt.value)
                ...
                if (dt.typ == DecreeToken.ItemType.OWNER): 
                    ...
                    if (dt.ref is not None and str(dt.ref.referent).startswith("агент")): 
                        dt = (None)
                    if (dt is not None): 
                        res1 = InstrToken._new1411(t0, dt.end_token, ILTypes.PERSON, dt.ref, dt.value)
                ...
                if (dpr.appendix is not None): 
                    if (t.is_newline_before or is_start_of_line): 
                        if (t.is_newline_after or t.whitespaces_before_count > 30): 
                            return InstrToken._new1409(t, t, ILTypes.APPENDIX, "ПРИЛОЖЕНИЕ")
                        ok = True
                        tt = t.next0_
                        first_pass2991 = True
                        while True:
            ...
            if (t.isValue("ЗА", None) and t.next0_ is not None and t.is_newline_before): 
                ...
                if ((isinstance(rr, PersonReferent)) or (isinstance(rr, PersonPropertyReferent)) or (isinstance(rr, InstrumentParticipant))): 
                    if (t != t0): 
                        break
                    res1 = InstrToken._new1405(t, t.next0_, ILTypes.PERSON, t.next0_)
                    t = t.next0_.next0_
                ...
                if (t.isValue(InstrToken._m_directives[ii], None)): 
                    if (t.next0_ is not None and t.next0_.isValue("СЛЕДУЮЩЕЕ", "НАСТУПНЕ")): 
                        ...
                        if (t11.next0_ is not None and t11.next0_.isCharOf(":.") and t11.next0_.is_newline_after): 
                            ...
                            t11 = t11.next0_
                        if (ok): 
                            return InstrToken._new1409(t, t11, ILTypes.DIRECTIVE, InstrToken._m_directives_norm[ii])
                    if (t.is_newline_after or ((t.next0_ is not None and t.next0_.isChar(':') and t.next0_.is_newline_after))): 
                        ...
                        if (not t.is_newline_before): 
                            if ((InstrToken._m_directives_norm[ii] != "ПРИКАЗ" and InstrToken._m_directives_norm[ii] != "ПОСТАНОВЛЕНИЕ" and InstrToken._m_directives_norm[ii] != "НАКАЗ") and InstrToken._m_directives_norm[ii] != "ПОСТАНОВУ"): 
                                break
                        return InstrToken._new1409(t, (t if t.is_newline_after else t.next0_), ILTypes.DIRECTIVE, InstrToken._m_directives_norm[ii])
                    break
                ii += 1
                ...
                if (npt is not None and npt.morph.case_.is_nominative and (isinstance(npt.end_token, TextToken))): 
                    ...
                    if (((term1 == "ПРИЛОЖЕНИЕ" or term1 == "ДОДАТОК" or term1 == "МНЕНИЕ") or term1 == "ДУМКА" or term1 == "АКТ") or term1 == "ФОРМА" or term == "ЗАЯВКА"): 
                        ...
                        if (tt1 is None): 
                            ...
                            ok = False
                        elif (tt1.isValue("К", None) and tt1.is_newline_before): 
                            return InstrToken._new1409(t, t, ILTypes.APPENDIX, term1)
            ...
            if (not t.chars.is_all_lower and t == t0): 
                ...
                if (tt is not None): 
                    if (tt.next0_ is not None and (isinstance(tt.next0_.getReferent(), DecreeReferent))): 
                        return InstrToken._new1405(t, tt, ILTypes.APPROVED, tt.next0_.getReferent())
    ...
    @staticmethod
    def _new1405(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ILTypes', _arg4 : object) -> 'InstrToken':
    ...
    @staticmethod
    def _new1406(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ILTypes') -> 'InstrToken':
        res = InstrToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new1409(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ILTypes', _arg4 : str) -> 'InstrToken':
        res = InstrToken(_arg1, _arg2)
        res.typ = _arg3
        res.value = _arg4
        return res
    ...
    @staticmethod
    def _new1411(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'ILTypes', _arg4 : object, _arg5 : str) -> 'InstrToken':
        res = InstrToken(_arg1, _arg2)
        res.typ = _arg3
        res.ref = _arg4
        res.value = _arg5
