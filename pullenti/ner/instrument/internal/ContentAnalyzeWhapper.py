...
class ContentAnalyzeWhapper:
    ...
    def __analizeContentWithContainers(self, root : 'FragToken', lines_ : typing.List['InstrToken1'], top_level : int, top_doc_ : 'FragToken') -> None:
        ...
        while True:
            ...
            if (li is None or (((n < len(nums)) and li == nums[n]))): 
                if (len(blk) > 0): 
                    if (fr is None): 
                        fr = FragToken._new1242(blk[0].begin_token, blk[len(blk) - 1].end_token, InstrumentKind.CONTENT)
                        if (len(blk) == 1): 
                            fr._itok = blk[0]
            ...
            if ((n < len(nums)) and li == nums[n]): 
                n += 1
                fr = FragToken._new1243(li.begin_token, li.end_token, li, li.is_expired)
                root.children.append(fr)
                ...
                if (li.begin_token != li.num_begin_token and li.num_begin_token is not None): 
                    fr.children.append(FragToken._new1244(li.begin_token, li.num_begin_token.previous, InstrumentKind.KEYWORD, True, li))
                ...
                if (li.num_end_token != li.end_token and li.num_end_token is not None): 
                    if (not li.all_upper and ((((li.has_verb and names == 0 and li.end_token.isCharOf(".:"))) or li.end_token.isChar(':')))): 
                        fr.children.append(FragToken._new1242(li.num_end_token.next0_, li.end_token, InstrumentKind.CONTENT))
                    else: 
                        fr_name = FragToken._new1244(li.num_end_token.next0_, li.end_token, InstrumentKind.NAME, True, li)
                    ...
                    fr_name = FragToken._new1244(li.begin_token, li.end_token, InstrumentKind.NAME, True, li)
                    fr.children.append(fr_name)
                    fr.name = FragToken._getRestoredNameMT(fr_name, False)
                    i = ContentAnalyzeWhapper.__correctName(fr, fr_name, lines_, i)
                    names += 1
            ...
            if (li.typ == InstrToken1.Types.EDITIONS and len(blk) == 0 and fr is not None): 
                fr.children.append(FragToken._new1242(li.begin_token, li.end_token, InstrumentKind.EDITIONS))
            else: 
                blk.append(li)
            ...
            if ((n < len(nums)) and li == nums[n]): 
                ...
                if (not li.all_upper and li.has_verb): 
                    if (((li.num_typ == NumberTypes.ROMAN and n >= 2 and len(childs) > 0) and childs[len(childs) - 1].kind == InstrumentKind.CHAPTER and li.num_typ == nums[n - 2].num_typ) and NumberingHelper.calcDelta(nums[n - 2], li, False) == 1): 
                        ...
                        blk.append(li)
                        continue
                fr = FragToken._new1251(li.begin_token, li.end_token, li)
                childs.append(fr)
                fr.kind = InstrumentKind.CHAPTER
                NumberingHelper.createNumber(fr, li)
                if (li.num_end_token != li.end_token and li.num_end_token is not None): 
                    if (li.has_many_spec_chars): 
                        fr.children.append(FragToken._new1252(li.num_end_token.next0_, li.end_token, InstrumentKind.CONTENT, li))
                    else: 
                        fr_name = FragToken._new1244(li.num_end_token.next0_, li.end_token, InstrumentKind.NAME, True, li)
                        fr.children.append(fr_name)
                        fr.name = FragToken._getRestoredNameMT(fr_name, False)
    ...
    def __addCommentOrEdition(self, fr : 'FragToken', li : 'InstrToken1') -> None:
        ...
        if (li.typ == InstrToken1.Types.COMMENT): 
            fr.children.append(FragToken._new1252(li.begin_token, li.end_token, InstrumentKind.COMMENT, li))
        elif (li.typ == InstrToken1.Types.EDITIONS): 
            edt = FragToken._new1252(li.begin_token, li.end_token, InstrumentKind.EDITIONS, li)
    ...
    def __analizeContentWithoutContainers(self, root : 'FragToken', lines_ : typing.List['InstrToken1'], is_subitem : bool, is_preamble : bool=False, is_kodex : bool=False) -> None:
                    ...
                    if ((ii > 0 and len(li.numbers) == 0 and li.typ != InstrToken1.Types.EDITIONS) and li.typ != InstrToken1.Types.COMMENT and part is not None): 
                        if (MiscHelper.canBeStartOfSentence(li.begin_token)): 
                            ...
                            if (end): 
                                ...
                                part = (None)
                    if (part is None): 
                        part = FragToken._new1259(li.begin_token, li.end_token, InstrumentKind.CLAUSEPART, len(parts) + 1)
                        parts.append(part)
        ...
        while ii < len(lines_): 
            if (lines_[ii].typ == InstrToken1.Types.NOTICE): 
                ...
                if (((len(li.numbers) == 1 and li.numbers[0] == "1")) or ((len(li.numbers) == 0 and ii == (len(lines_) - 1)))): 
                    ...
                    while j < len(lines_): 
                        li = lines_[j]
                        not0_ = FragToken._new1252(li.begin_token, li.end_token, InstrumentKind.NOTICE, li)
                        notices.append(not0_)
                        if (li.num_begin_token is not None and li.begin_token != li.num_begin_token): 
                            not0_.children.append(FragToken._new1261(li.begin_token, li.num_begin_token.previous, InstrumentKind.KEYWORD, True))
                        if (len(li.numbers) > 0): 
                            NumberingHelper.createNumber(not0_, li)
                        if (len(not0_.children) > 0): 
                            not0_.children.append(FragToken._new1252(Utils.ifNotNull(li.num_end_token, li.begin_token), li.end_token, InstrumentKind.CONTENT, li))
                        j += 1
        ...
        if (nums is None): 
            ...
            for li in lines_: 
                ...
                if (li.typ == InstrToken1.Types.INDEX): 
                    ind = FragToken._new1252(li.begin_token, li.end_token, InstrumentKind.INDEX, li)
                    ...
                    if (not li.index_no_keyword): 
                        while tt is not None and tt.end_char <= li.end_char: 
                            if (tt.is_newline_after): 
                                ind.children.append(FragToken._new1264(li.begin_token, tt, InstrumentKind.NAME, True))
                                tt = tt.next0_
                                break
                            tt = tt.next0_
                    is_tab = False
                    ...
                    while True:
                        ...
                        if (isinstance(tt.getReferent(), DecreePartReferent)): 
                            ...
                            it1 = InstrToken1.parse(tt, True, None, 0, None, False, 0, False)
                        ind_item = FragToken._new1242(tt, it1.end_token, InstrumentKind.INDEXITEM)
                        ...
                        if (it1.num_end_token is not None and it1.num_end_token != it1.end_token): 
                            if (it1.begin_token != it1.num_begin_token): 
                                ind_item.children.append(FragToken._new1261(it1.begin_token, it1.num_begin_token.previous, InstrumentKind.KEYWORD, True))
                            NumberingHelper.createNumber(ind_item, it1)
                            nam = FragToken._new1261(it1.num_end_token.next0_, it1.end_token, InstrumentKind.NAME, True)
                            ...
                            nam = FragToken._new1261(it1.begin_token, it1.end_token, InstrumentKind.NAME, True)
                            ind_item.children.append(nam)
                ...
                if (last is not None and last.kind == InstrumentKind.CONTENT): 
                    ...
                    last = FragToken._new1252(li.begin_token, li.end_token, InstrumentKind.CONTENT, li)
        ...
        while i < len(lines_): 
            ...
            if (li.num_end_token != li.end_token and li.num_end_token is not None): 
                ...
                fr.children.append(FragToken._new1264(li.begin_token, li.end_token, InstrumentKind.NAME, True))
            if (len(blk) > 0): 
                fr.end_token = blk[len(blk) - 1].end_token
            ...
            if (parts[0].name is None and parts[len(parts) - 1].name is not None): 
                if (parts[1].name == "МОТИВИРОВОЧНАЯ" and not parts[0].is_newline_after): 
                    ...
                    if (len(parts[0].children) == 0): 
                        parts[0].children.append(FragToken._new1252(parts[0].begin_token, parts[0].end_token, InstrumentKind.CONTENT, parts[0]._itok))
            i = 0
            while i < (len(parts) - 1): 
    ...
    def __createDirectivePart(self, lines_ : typing.List['InstrToken1']) -> 'FragToken':
        from pullenti.ner.instrument.internal.FragToken import FragToken
        res = FragToken._new1242(lines_[0].begin_token, lines_[len(lines_) - 1].end_token, InstrumentKind.DOCPART)
        head = list()
        i = 0
        while i < len(lines_): 
            if (lines_[i].typ == InstrToken1.Types.DIRECTIVE): 
        ...
        if (i >= len(lines_)): 
            ...
            return res
        if (len(head) > 0): 
            fr_head = FragToken._new1242(head[0].begin_token, head[len(head) - 1].end_token, InstrumentKind.CONTENT)
        ...
        res.children.append(FragToken._new1276(lines_[i].begin_token, lines_[i].end_token, InstrumentKind.DIRECTIVE, lines_[i].value, lines_[i]))
        vvv = lines_[i].value
            ...
            for ch in root.children: 
                if (ch.kind == InstrumentKind.CHAPTER or ch.kind == InstrumentKind.CLAUSE): 
                    ...
                    if (ch.children[i].kind != InstrumentKind.CONTENT): 
                        ...
                        sect = FragToken._new1261(ch.children[i].begin_token, ch.children[i].end_token, InstrumentKind.SECTION, True)
                        sect.name = (Utils.asObjectOrNull(sect.value, str))
                        sect.value = None
                        sect.children.append(FragToken._new1242(sect.begin_token, sect.end_token, InstrumentKind.NAME))
                        ...
                        while True:
                            ...
                            if (tt.is_newline_before): 
                                ...
                                if ((tt.previous.end_char - la.begin_char) < 20): 
                                    break
                                next_sect = FragToken._new1261(tt, la.end_token, InstrumentKind.SECTION, True)
                                next_sect.name = (Utils.asObjectOrNull(next_sect.value, str))
                                next_sect.value = None
                                next_sect.children.append(FragToken._new1242(tt, la.end_token, InstrumentKind.NAME))
                                break
    ...
    def __splitContentByIndents(fr : 'FragToken', num : int) -> typing.List['FragToken']:
        ...
        if (fr.kind != InstrumentKind.CONTENT and fr.kind != InstrumentKind.LISTITEM and fr.kind != InstrumentKind.PREAMBLE): 
            ...
            if (fr.begin_token.isValue("АБЗАЦ", None)): 
                ...
                i = num
                while i <= next0_: 
                    res0.append(FragToken._new1281(fr.begin_token, fr.end_token, InstrumentKind.INDENTION, i, True, fr.referents))
                    i += 1
                return res0
            return list()
        if (len(fr.children) > 0): 
        ...
        while True:
            ...
            if (tt.end_char == fr.end_char): 
                ...
                continue
            re = FragToken._new1259(t0, tt, InstrumentKind.INDENTION, num)
            num += 1
    ...
    def __analizePreamble(self, root : 'FragToken') -> None:
                    ...
                    if (t.is_newline_before): 
                        if (t.previous.isCharOf(".:;") and t.previous.previous is not None and ((t.previous.previous.isValue("НИЖЕСЛЕДУЮЩИЙ", None) or t.previous.previous.isValue("ДОГОВОР", None)))): 
                            ...
                            if (itt1 is not None and not itt1.has_verb and (itt1.end_char < ch.end_char)): 
                                clau = FragToken._new1242(t, ch.end_token, InstrumentKind.CHAPTER)
                                if (((i + 1) < len(root.children)) and root.children[i + 1].kind == InstrumentKind.CLAUSE): 
                                    clau.kind = InstrumentKind.CLAUSE
                                nam = FragToken._new1244(t, itt1.end_token, InstrumentKind.NAME, True, itt1)
                                clau.children.append(nam)
                                clau.name = FragToken._getRestoredNameMT(nam, False)
                                clau.children.append(FragToken._new1242(itt1.end_token.next0_, ch.end_token, InstrumentKind.CONTENT))
        ...
        if (cnt_cou == 1): 
            ...
            prm = FragToken._new1242(root.children[0].begin_token, root.children[i - 1].end_token, InstrumentKind.PREAMBLE)
            j = 0
            while j < i: 
