...
class ListHelper:
                        ...
                        if ((ord(list_bullet)) != 0 and ch.begin_token.isChar(list_bullet)): 
                            ...
                            while tt is not None and (tt.end_char < ch.end_char): 
                                if (tt.previous.isChar('.') and MiscHelper.canBeStartOfSentence(tt)): 
                                    ch2 = FragToken._new1259(tt, ch.end_token, InstrumentKind.INDENTION, ch.number)
    ...
    def __analizeListItems(chi : typing.List['FragToken'], ind : int) -> int:
        ...
        if (res.has_changes and res.multiline_changes_value is not None): 
            ...
            cit = FragToken._new1242(ci.begin_token, ci.end_token, InstrumentKind.CITATION)
        ...
        for li in lines: 
            ...
            ch = FragToken._new1259(li.begin_token, li.end_token, (InstrumentKind.LISTITEM if li.is_list_item else InstrumentKind.CONTENT), li.number)
            ...
            if (chli is not None): 
                for lt in chli: 
                    ch.children.append(FragToken._new1242(lt.begin_token, lt.end_token, InstrumentKind.LISTITEM))
                if (ch.begin_char < ch.children[0].begin_char): 
                    ch.children.insert(0, FragToken._new1242(ch.begin_token, ch.children[0].begin_token.previous, InstrumentKind.CONTENT))
        return ret
            ...
            if ((lines[i].typ == InstrToken1.Types.LINE and len(lines[i].numbers) == 0 and lines[i].begin_token.isValue("ПРИЛОЖЕНИЯ", "ДОДАТОК")) and len(lines[i + 1].numbers) > 0 and lines[i].end_token.isChar(':')): 
                ...
                while True:
                    ...
                    if (len(lines[i].numbers) == 0): 
                        ...
                        wrapnn1447 = RefOutArgWrapper(0)
                        inoutres1448 = Utils.tryParseInt(lines[i].numbers[0], wrapnn1447)
                        nn = wrapnn1447.value
                        if (inoutres1448): 
                            num = (nn + 1)
    ...
    def correctIndex(lines : typing.List['InstrToken1']) -> None:
        ...
        if (len(index) == len(content) and len(index) > 2): 
            if ((ind_text * 10) < con_text): 
                lines[0] = InstrToken1._new1449(lines[0].begin_token, lines[cind - 1].end_token, True, InstrToken1.Types.INDEX)
