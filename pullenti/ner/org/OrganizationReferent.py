...
class OrganizationReferent(Referent):
    ...
    def __correctName(self, name : str, num : int) -> str:
        ...
        if (str.isdigit(name[0]) and name.find(' ') > 0): 
            wrapi2235 = RefOutArgWrapper(0)
            inoutres2236 = Utils.tryParseInt(name[0:0+name.find(' ')], wrapi2235)
            i = wrapi2235.value
            if (inoutres2236): 
                if (i > 1): 
                    num.value = i
            ...
            if (i >= 0 and name[i] == '.'): 
                ...
                inoutres2237 = Utils.tryParseInt(name[i + 1:], num)
                if (i > 0 and inoutres2237 and num.value > 0): 
                    if (i < 1): 
                        return None
    ...
    def addName(self, name : str, remove_long_gov_names : bool=True, t : 'Token'=None) -> None:
        wrapnum2238 = RefOutArgWrapper(0)
        s = self.__correctName(name, wrapnum2238)
        num = wrapnum2238.value
        if (s is None): 
    ...
    def addNameStr(self, name : str, typ : 'OrgItemTypeToken', cou : int=1) -> None:
        ...
        if (name is None): 
            ...
            if (typ.name is not None and len(typ.name) > len(typ.typ)): 
                num = 0
                wrapnum2239 = RefOutArgWrapper(0)
                s = self.__correctName(typ.name, wrapnum2239)
                num = wrapnum2239.value
            ...
            if (typ is None or typ.is_not_typ): 
                ...
                if (typ.name is not None): 
                    num = 0
                    wrapnum2240 = RefOutArgWrapper(0)
                    ss = self.__correctName(typ.name, wrapnum2240)
                    num = wrapnum2240.value
                    if (ss is not None): 
                        self.addTypeStr(ss)
            ...
            if (((eq_number or eq_eponyms)) and ((self.findSlot(OrganizationReferent.ATTR_NAME, None, True) is None or org0_.findSlot(OrganizationReferent.ATTR_NAME, None, True) is None))): 
                ...
                for v in self._name_vars.items(): 
                    if (typ == Referent.EqualType.DIFFERENTTEXTS and v[1]): 
                        continue
                    wrapb2242 = RefOutArgWrapper(False)
                    inoutres2243 = Utils.tryGetValue(org0_._name_vars, v[0], wrapb2242)
                    b = wrapb2242.value
                    if (not inoutres2243): 
                        continue
