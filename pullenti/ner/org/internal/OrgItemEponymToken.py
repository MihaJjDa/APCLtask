...
class OrgItemEponymToken(MetaToken):
    ...
    class PersonItemToken(MetaToken):
            ...
            while True:
                ...
                if (((len(s) == 1 or s == "ДЖ")) and not tt.chars.is_all_lower): 
                    ...
                    if (t1.next0_ is not None and t1.next0_.isChar('.')): 
                        t1 = t1.next0_
                    res.append(OrgItemEponymToken.PersonItemToken._new1678(t, t1, OrgItemEponymToken.PersonItemType.INITIAL, s))
                    t = t1
                    continue
                if (tt.is_and): 
                    res.append(OrgItemEponymToken.PersonItemToken._new1679(t, t, OrgItemEponymToken.PersonItemType.AND))
                    continue
                ...
                if (tt.chars.is_all_lower): 
                    ...
                    if (t1.next0_ is not None and not t1.is_whitespace_after and t1.next0_.isChar('.')): 
                        t1 = t1.next0_
                    res.append(OrgItemEponymToken.PersonItemToken._new1678(t, t1, OrgItemEponymToken.PersonItemType.LOCASEWORD, s))
                    t = t1
                    continue
                if (tt.morph.class0_.is_proper_name): 
                    res.append(OrgItemEponymToken.PersonItemToken._new1678(t, t, OrgItemEponymToken.PersonItemType.NAME, s))
                    ...
                    res.append(OrgItemEponymToken.PersonItemToken._new1678(t, t.next0_.next0_, OrgItemEponymToken.PersonItemType.SURNAME, "{0}-{1}".format(s, (t.next0_.next0_).term)))
                    t = t.next0_.next0_
                else: 
                    res.append(OrgItemEponymToken.PersonItemToken._new1678(t, t, OrgItemEponymToken.PersonItemType.SURNAME, s))
            return (res if len(res) > 0 else None)
        
        @staticmethod
        def _new1678(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PersonItemType', _arg4 : str) -> 'PersonItemToken':
        ...
        @staticmethod
        def _new1679(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'PersonItemType') -> 'PersonItemToken':
            res = OrgItemEponymToken.PersonItemToken(_arg1, _arg2)
            res.typ = _arg3
            return res
    ...
    def tryAttach(t : 'Token', must_has_prefix : bool=False) -> 'OrgItemEponymToken':
        ...
        if (tt is None): 
            ...
            if (r1 is not None and r1.type_name == "DATE"): 
                ...
                if ((str0_ == "1 МАЯ" or str0_ == "7 ОКТЯБРЯ" or str0_ == "9 МАЯ") or str0_ == "8 МАРТА"): 
                    dt = OrgItemEponymToken._new1674(t, t, list())
            ...
            if ((age is not None and (((isinstance(age.end_token.next0_, TextToken)) or (isinstance(age.end_token.next0_, ReferentToken)))) and (age.whitespaces_after_count < 3)) and not age.end_token.next0_.chars.is_all_lower and age.end_token.next0_.chars.is_cyrillic_letter): 
                dt = OrgItemEponymToken._new1674(t, age.end_token.next0_, list())
        ...
        if ((r is not None and r.type_name == "DATE" and full) and r.findSlot("DAY", None, True) is not None and r.findSlot("YEAR", None, True) is None): 
            dt = OrgItemEponymToken._new1674(t, t1, list())
        ...
        if (len(eponims) == 0): 
            return None
        return OrgItemEponymToken._new1674(t, t1, eponims)
    
    @staticmethod
    def _new1674(_arg1 : 'Token', _arg2 : 'Token', _arg3 : typing.List[str]) -> 'OrgItemEponymToken':
