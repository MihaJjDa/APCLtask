...
class PersonIdToken(MetaToken):
    ...
    def __tryParse(t : 'Token', prev : 'PersonIdToken') -> 'PersonIdToken':
        if (t.isValue("СВИДЕТЕЛЬСТВО", None)): 
            ...
            while True:
                ...
                if (tt.isValue("РЕГИСТРАЦИЯ", None) or tt.isValue("РЕЕСТР", None) or tt.isValue("ЗАРЕГИСТРИРОВАТЬ", None)): 
                    ...
                    break
            if (reg and ip): 
                return PersonIdToken._new2350(t, tt1, PersonIdToken.Typs.KEYWORD, "СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ")
        tok = PersonIdToken.M_ONTOLOGY.tryParse(t, TerminParseAttr.NO)
        if (tok is not None): 
            ty = Utils.valToEnum(tok.termin.tag, PersonIdToken.Typs)
            res = PersonIdToken._new2350(tok.begin_token, tok.end_token, ty, tok.termin.canonic_text)
            if (prev is None): 
        ...
        if (isinstance(t, NumberToken)): 
            tmp = io.StringIO()
            res = PersonIdToken._new2352(t0, t, PersonIdToken.Typs.NUMBER)
            tt = t
            while tt is not None: 
        ...
        if (isinstance(t, ReferentToken)): 
            ...
            if (r is not None): 
                if (r.type_name == "DATE"): 
                    return PersonIdToken._new2353(t, t, PersonIdToken.Typs.DATE, r)
                if (r.type_name == "ORGANIZATION"): 
                    return PersonIdToken._new2353(t, t, PersonIdToken.Typs.ORG, r)
                if (r.type_name == "ADDRESS"): 
                    return PersonIdToken._new2353(t, t, PersonIdToken.Typs.ADDRESS, r)
        if ((prev is not None and prev.typ == PersonIdToken.Typs.KEYWORD and (isinstance(t, TextToken))) and not t.chars.is_all_lower and t.chars.is_letter): 
            ...
            if (rr is not None and rr.typ == PersonIdToken.Typs.NUMBER): 
                return PersonIdToken._new2350(t, t, PersonIdToken.Typs.SERIA, (t).term)
        if ((t is not None and t.isValue("ОТ", "ВІД") and (isinstance(t.next0_, ReferentToken))) and t.next0_.getReferent().type_name == "DATE"): 
            return PersonIdToken._new2353(t, t.next0_, PersonIdToken.Typs.DATE, t.next0_.getReferent())
        return None
    ...
    def initialize() -> None:
        ...
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new117("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ", PersonIdToken.Typs.KEYWORD)
        t.addVariant("УДОСТОВЕРЕНИЕ ЛИЧНОСТИ ОФИЦЕРА", False)
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new117("СВИДЕТЕЛЬСТВО О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ ФИЗИЧЕСКОГО ЛИЦА В КАЧЕСТВЕ ИНДИВИДУАЛЬНОГО ПРЕДПРИНИМАТЕЛЯ", PersonIdToken.Typs.KEYWORD)
        ...
        PersonIdToken.M_ONTOLOGY.add(t)
        t = Termin._new117("ВОДИТЕЛЬСКОЕ УДОСТОВЕРЕНИЕ", PersonIdToken.Typs.KEYWORD)
        PersonIdToken.M_ONTOLOGY.add(t)
    ...
    @staticmethod
    def _new2350(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs', _arg4 : str) -> 'PersonIdToken':
    ...
    @staticmethod
    def _new2352(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs') -> 'PersonIdToken':
        res = PersonIdToken(_arg1, _arg2)
        res.typ = _arg3
        return res
    
    @staticmethod
    def _new2353(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'Typs', _arg4 : 'Referent') -> 'PersonIdToken':
        res = PersonIdToken(_arg1, _arg2)
        res.typ = _arg3
        res.referent = _arg4
        return res