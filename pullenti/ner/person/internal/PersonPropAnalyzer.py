...
class PersonPropAnalyzer(Analyzer):
    ...
    def _processReferent(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        pat = PersonAttrToken.tryAttach(begin, None, PersonAttrToken.PersonAttrAttachAttrs.INPROCESS)
        if (pat is not None and pat.prop_ref is not None): 
            return ReferentToken._new2438(pat.prop_ref, pat.begin_token, pat.end_token, pat.morph, pat)
        return None