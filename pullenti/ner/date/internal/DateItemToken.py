...
class DateItemToken(MetaToken):
    ...
    def __TryAttach(t : 'Token', prev : typing.List['DateItemToken']) -> 'DateItemToken':
        ...
        if (nt is not None): 
            ...
            if (NumberHelper.tryParseAge(nt) is not None): 
                return None
            res = DateItemToken._new653(begin, end, DateItemToken.DateItemType.NUMBER, nt.int_value, nt.morph)
                ...
                if (nt.previous.isValue("В", "У") or nt.previous.isValue("К", None) or nt.previous.isValue("ДО", None)): 
                    ...
                    if ((tt) is not None): 
                        ...
                        if ((res.int_value < 100) and (isinstance(tt, TextToken)) and (((tt).term == "ГОДА" or (tt).term == "РОКИ"))): 
                            ...
                            ok = True
                            if (nt.previous.isValue("ДО", None) and nt.next0_.isValue("Г", None)): 
                                cou = 0
                                ttt = nt.previous.previous
                                while ttt is not None and (cou < 10): 
                                    mt = MeasureToken.tryParse(ttt, None, False, False)
        ...
        if ((txt[0] == 'I' or txt[0] == 'X' or txt[0] == 'Х') or txt[0] == 'V'): 
            ...
            if (lat is not None and lat.end_token.next0_ is not None and lat.int_value is not None): 
                ...
                if (tt.isValue("КВАРТАЛ", None) and val > 0 and val <= 4): 
                    return DateItemToken._new654(t, tt, DateItemToken.DateItemType.QUARTAL, val)
                if (tt.isValue("ПОЛУГОДИЕ", "ПІВРІЧЧЯ") and val > 0 and val <= 2): 
                    return DateItemToken._new654(t, lat.end_token.next0_, DateItemToken.DateItemType.HALFYEAR, val)
                if (tt.isValue("ВЕК", "ВІК") or tt.isValue("СТОЛЕТИЕ", "СТОЛІТТЯ")): 
                    return DateItemToken._new654(t, lat.end_token.next0_, DateItemToken.DateItemType.CENTURY, val)
                if (tt.isValue("В", None) and tt.next0_ is not None and tt.next0_.isChar('.')): 
                    if (prev is not None and len(prev) > 0 and prev[len(prev) - 1].typ == DateItemToken.DateItemType.POINTER): 
                        return DateItemToken._new654(t, tt.next0_, DateItemToken.DateItemType.CENTURY, val)
                ...
                if (tt.is_hiphen): 
                    ...
                    if ((lat2 is not None and lat2.int_value is not None and lat2.int_value > val) and lat2.end_token.next0_ is not None): 
                        if (lat2.end_token.next0_.isValue("ВЕК", "ВІК") or lat2.end_token.next0_.isValue("СТОЛЕТИЕ", "СТОЛІТТЯ")): 
                            return DateItemToken._new654(t, lat.end_token, DateItemToken.DateItemType.CENTURY, val)
        if (t is not None and t.isValue("НАПРИКІНЦІ", None)): 
            return DateItemToken._new660(t, t, DateItemToken.DateItemType.POINTER, "конец")
        if (t is not None and t.isValue("ДОНЕДАВНА", None)): 
            return DateItemToken._new660(t, t, DateItemToken.DateItemType.POINTER, "сегодня")
        ...
        if ((tok is not None and (Utils.valToEnum(tok.termin.tag, DatePointerType)) == DatePointerType.SUMMER and t.morph.language.is_ru) and (isinstance(t, TextToken))): 
            ...
            if (str0_ != "ЛЕТОМ" and str0_ != "ЛЕТА" and str0_ != "ЛЕТО"): 
                tok = (None)
        if (tok is not None): 
            return DateItemToken._new654(t, tok.end_token, DateItemToken.DateItemType.POINTER, Utils.valToEnum(tok.termin.tag, DatePointerType))
        ...
        if (npt is not None): 
            ...
            if (npt.noun.isValue("КВАРТАЛ", None)): 
                ...
                return DateItemToken._new660(t, npt.end_token, DateItemToken.DateItemType.POINTER, "начало")
            elif (npt.end_token.isValue("СЕРЕДИНА", None)): 
                return DateItemToken._new660(t, npt.end_token, DateItemToken.DateItemType.POINTER, "середина")
            elif (npt.end_token.isValue("КОНЕЦ", None) or npt.end_token.isValue("КІНЕЦЬ", None) or npt.end_token.isValue("НАПРИКІНЕЦЬ", None)): 
                return DateItemToken._new660(t, npt.end_token, DateItemToken.DateItemType.POINTER, "конец")
            ...
            if (typ_ != DateItemToken.DateItemType.NUMBER): 
                ...
                if (len(npt.adjectives) > 0): 
                    if (npt.adjectives[0].isValue("ПОСЛЕДНИЙ", None) or npt.adjectives[0].isValue("ОСТАННІЙ", None)): 
                        return DateItemToken._new654(t0, npt.end_token, typ_, (4 if typ_ == DateItemToken.DateItemType.QUARTAL else 2))
                ...
                while True:
                    ...
                    if (typ_ == DateItemToken.DateItemType.QUARTAL): 
                        ...
                        if ((ii < 1) or ii > 4): 
                            continue
                        return DateItemToken._new654(t0, npt.end_token, typ_, ii)
        ...
        if (not str.isalnum(term[0])): 
            if (t0.isCharOf(".\\/:") or t0.is_hiphen): 
                return DateItemToken._new660(t0, t0, DateItemToken.DateItemType.DELIM, term)
            elif (t0.isChar(',')): 
                return DateItemToken._new660(t0, t0, DateItemToken.DateItemType.DELIM, term)
            else: 
                return None
        if (term == "O" or term == "О"): 
            if ((isinstance(t.next0_, NumberToken)) and not t.is_whitespace_after and len((t.next0_).value) == 1): 
                return DateItemToken._new654(t, t.next0_, DateItemToken.DateItemType.NUMBER, (t.next0_).int_value)
        if (str.isalpha(term[0])): 
            ...
            if (inf is not None and (isinstance(inf.termin.tag, int))): 
                return DateItemToken._new675(inf.begin_token, inf.end_token, DateItemToken.DateItemType.MONTH, inf.termin.tag, inf.termin.lang)
        return None
    ...
    def initialize() -> None:
        ...
        DateItemToken.M_SEASONS.add(Termin._new678("ЗИМА", MorphLang.RU, True, DatePointerType.WINTER))
        DateItemToken.M_SEASONS.add(Termin._new678("WINTER", MorphLang.EN, True, DatePointerType.WINTER))
        t = Termin._new678("ВЕСНА", MorphLang.RU, True, DatePointerType.SPRING)
        t.addVariant("ПРОВЕСНА", True)
        DateItemToken.M_SEASONS.add(t)
        DateItemToken.M_SEASONS.add(Termin._new678("SPRING", MorphLang.EN, True, DatePointerType.SPRING))
        ...
        DateItemToken.M_SEASONS.add(t)
        t = Termin._new678("AUTUMN", MorphLang.EN, True, DatePointerType.AUTUMN)
        DateItemToken.M_SEASONS.add(t)
    ...
    def tryAttachList(t : 'Token', max_count : int=20) -> typing.List['DateItemToken']:
        ...
        if (len(res) > 0 and res[len(res) - 1].typ == DateItemToken.DateItemType.NUMBER): 
            nex = NumberHelper.tryParseNumberWithPostfix(res[len(res) - 1].begin_token)
    ...
    @staticmethod
    def _new653(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : 'MorphCollection') -> 'DateItemToken':
    ...
    @staticmethod
    def _new654(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        return res
    ...
    @staticmethod
    def _new660(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : str) -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
    ...
    @staticmethod
    def _new675(_arg1 : 'Token', _arg2 : 'Token', _arg3 : 'DateItemType', _arg4 : int, _arg5 : 'MorphLang') -> 'DateItemToken':
        res = DateItemToken(_arg1, _arg2)
        res.typ = _arg3
        res.int_value = _arg4
        res.lang = _arg5
