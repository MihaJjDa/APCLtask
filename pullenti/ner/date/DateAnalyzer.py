...
class DateAnalyzer(Analyzer):
    ...
    class DateAnalizerData(AnalyzerData):
        ...
        def registerReferent(self, referent : 'Referent') -> 'Referent':
            key = str(referent)
            wrapdr698 = RefOutArgWrapper(None)
            inoutres699 = Utils.tryGetValue(self.__m_hash, key, wrapdr698)
            dr = wrapdr698.value
            if (inoutres699): 
                return dr
            ...
            if (rts is not None): 
                ...
                while i < len(rts): 
                    ...
                    if ((i < (len(rts) - 1)) and rt.tag is None): 
                        rt.referent.addOccurence(TextAnnotation._new700(kit.sofa, rt.begin_char, rt.end_char, rt.referent))
                    else: 
                ...
                if ((dat is not None and t.previous is not None and t.previous.is_hiphen) and t.previous.previous is not None and (isinstance(t.previous.previous.getReferent(), DateReferent))): 
                    ...
                    dr = Utils.asObjectOrNull(ad.registerReferent(DateRangeReferent._new701(dat0, dat)), DateRangeReferent)
                    diap = ReferentToken(dr, t.previous.previous, t)
                    kit.embedToken(diap)
                    t = (diap)
                    continue
                if ((dat is not None and t.previous is not None and ((t.previous.is_hiphen or t.previous.isValue("ПО", None) or t.previous.isValue("И", None)))) and (isinstance(t.previous.previous, NumberToken)) and (t.previous.previous).int_value is not None): 
                    t0 = t.previous.previous
                    dat0 = None
                    num = (t0).int_value
                    if (dat.day > 0 and (num < dat.day) and num > 0): 
                        if (dat.higher is not None): 
                            dat0 = DateReferent._new702(dat.higher, num)
                        elif (dat.month > 0): 
                            dat0 = DateReferent._new703(dat.month, num)
    ...
    def _processReferent(self, begin : 'Token', end : 'Token') -> 'ReferentToken':
        ...
        if (begin.isValue("ДО", None) and (isinstance(begin.next0_, ReferentToken)) and (isinstance(begin.next0_.getReferent(), DateReferent))): 
            drr = DateRangeReferent._new706(Utils.asObjectOrNull(begin.next0_.getReferent(), DateReferent))
    ...
    def __tryAttach(self, dts : typing.List['DateItemToken'], high : bool) -> typing.List['ReferentToken']:
            ...
            if (dts[0].typ == DateItemToken.DateItemType.HOUR or ((dts[0].typ == DateItemToken.DateItemType.NUMBER and ((dts[2].typ == DateItemToken.DateItemType.HOUR or dts[2].typ == DateItemToken.DateItemType.NUMBER))))): 
                if (len(dts) > 3 and dts[3].typ == DateItemToken.DateItemType.DELIM and dts[3].string_value == dts[1].string_value): 
                    ...
                    if (res1 is not None and (isinstance(res1[len(res1) - 1].referent, DateReferent)) and (res1[len(res1) - 1].referent).day > 0): 
                        time = DateReferent._new707(dts[0].int_value, dts[2].int_value)
        ...
        year_is_dif = False
        b = False
        wrapyear752 = RefOutArgWrapper(None)
        wrapmon753 = RefOutArgWrapper(None)
        wrapday754 = RefOutArgWrapper(None)
        b = self.__applyRuleFormal(dts, high, wrapyear752, wrapmon753, wrapday754)
        ...
        if (len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.POINTER and dts[0].string_value == "сегодня"): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new708(DatePointerType.TODAY), dts[0].begin_token, dts[0].end_token))
            return res0
        if (len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.YEAR and dts[0].year <= 0): 
            res0 = list()
            res0.append(ReferentToken(DateReferent._new708(DatePointerType.UNDEFINED), dts[0].begin_token, dts[0].end_token))
            return res0
        if (not b and dts[0].typ == DateItemToken.DateItemType.POINTER and len(dts) > 1): 
            if (dts[1].typ == DateItemToken.DateItemType.YEAR): 
                ...
                b = True
        if (not b): 
            wrapyear710 = RefOutArgWrapper(None)
            wrapmon711 = RefOutArgWrapper(None)
            wrapday712 = RefOutArgWrapper(None)
            wrapyear_is_dif713 = RefOutArgWrapper(False)
            b = self.__applyRuleWithMonth(dts, high, wrapyear710, wrapmon711, wrapday712, wrapyear_is_dif713)
            year = wrapyear710.value
            mon = wrapmon711.value
            day = wrapday712.value
            year_is_dif = wrapyear_is_dif713.value
        if (not b): 
            wrapyear714 = RefOutArgWrapper(None)
            wrapmon715 = RefOutArgWrapper(None)
            wrapday716 = RefOutArgWrapper(None)
            b = self.__applyRuleYearOnly(dts, wrapyear714, wrapmon715, wrapday716)
        ...
        if (not b): 
            if (len(dts) == 2 and dts[0].typ == DateItemToken.DateItemType.HOUR and dts[1].typ == DateItemToken.DateItemType.MINUTE): 
                ...
                if (t00 is not None and (((t00.isValue("ТЕЧЕНИЕ", None) or t00.isValue("ПРОТЯГОМ", None) or t00.isValue("ЧЕРЕЗ", None)) or t00.isValue("ТЕЧІЮ", None)))): 
                    ...
                    res0 = list()
                    time = DateReferent._new707(dts[0].int_value, dts[1].int_value)
            ...
            if ((len(dts) == 4 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[1].typ == DateItemToken.DateItemType.DELIM) and dts[2].typ == DateItemToken.DateItemType.MONTH and dts[3].typ == DateItemToken.DateItemType.YEAR): 
                res0 = list()
                yea = DateReferent._new704(dts[3].int_value)
                res0.append(ReferentToken._new719(yea, dts[3].begin_token, dts[3].end_token, dts[3].morph))
                mon1 = DateReferent._new720(dts[0].int_value, yea)
                res0.append(ReferentToken._new721(mon1, dts[0].begin_token, dts[0].end_token, mon1))
                mon2 = DateReferent._new720(dts[2].int_value, yea)
            ...
            if ((dts[0].typ == DateItemToken.DateItemType.MONTH and len(dts) == 1 and dts[0].end_token.next0_ is not None) and ((dts[0].end_token.next0_.is_hiphen or dts[0].end_token.next0_.isValue("ПО", None) or dts[0].end_token.next0_.isValue("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if ((dr0 is not None and dr0.year > 0 and dr0.month > 0) and dr0.day == 0 and dr0.month > dts[0].int_value): 
                        dr_year0 = DateReferent._new704(dr0.year)
                        res0 = list()
                        res0.append(ReferentToken(dr_year0, dts[0].end_token, dts[0].end_token))
                        dr_mon0 = DateReferent._new720(dts[0].int_value, dr_year0)
            ...
            if (((len(dts) == 3 and dts[1].typ == DateItemToken.DateItemType.DELIM and dts[1].begin_token.is_hiphen) and dts[0].can_be_year and dts[2].can_be_year) and (dts[0].int_value < dts[2].int_value)): 
                ...
                if (ok): 
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new704(dts[0].year), dts[0].begin_token, dts[0].end_token))
                    res0.append(ReferentToken(DateReferent._new704(dts[2].year), dts[2].begin_token, dts[2].end_token))
                    return res0
            ...
            if (dts[0].typ == DateItemToken.DateItemType.QUARTAL or dts[0].typ == DateItemToken.DateItemType.HALFYEAR): 
                if (len(dts) == 1 or dts[1].typ == DateItemToken.DateItemType.YEAR): 
                    ...
                    if (len(dts) > 1): 
                        ii = 1
                        yea = DateReferent._new704(dts[1].int_value)
                        res0.append(ReferentToken._new719(yea, dts[1].begin_token, dts[1].end_token, dts[1].morph))
                    else: 
                        cou = 0
            ...
            if ((len(dts) == 1 and dts[0].typ == DateItemToken.DateItemType.MONTH and dts[0].begin_token.previous is not None) and dts[0].begin_token.previous.morph.class0_.is_preposition): 
                if (dts[0].chars.is_latin_letter and dts[0].chars.is_all_lower): 
                    ...
                    res0 = list()
                    res0.append(ReferentToken(DateReferent._new741(dts[0].int_value), dts[0].begin_token, dts[0].end_token))
                    return res0
            return None
        res = list()
        dr_year = None
        ...
        t1 = None
        if (cent is not None): 
            ce = DateReferent._new742((- cent.int_value if cent.new_age < 0 else cent.int_value))
            t1 = cent.end_token
            rt = ReferentToken(ce, cent.begin_token, t1)
            res.append(rt)
        if (year is not None and year.year > 0): 
            dr_year = DateReferent._new704((- year.year if year.new_age < 0 else year.year))
            if (not year_is_dif): 
                t1 = year.end_token
                if (t1.next0_ is not None and t1.next0_.isValue("ГОРОД", None)): 
                    ...
                    if (tt2 is None): 
                        ...
                        year.end_token = t1
            t0 = year.begin_token
            res.append(ReferentToken._new719(dr_year, t0, year.end_token, year.morph))
        if (mon is not None): 
            dr_mon = DateReferent._new741(mon.int_value)
            ...
            if (dr_year is None and t1.next0_ is not None and ((t1.next0_.isValue("ПО", None) or t1.next0_.isValue("НА", None)))): 
                ...
                if (rt is not None): 
                    ...
                    if (dr0 is not None and dr0.year > 0 and dr0.month > 0): 
                        dr_year = DateReferent._new704(dr0.year)
                        t0 = t1
                        res.append(ReferentToken(dr_year, t0, t1))
                        dr_mon.higher = dr_year
            res.append(ReferentToken._new719(dr_mon, t0, t1, mon.morph))
            if (day is not None): 
                dr_day = DateReferent._new748(day.int_value)
            ...
            if (dt0.month == 0): 
                ...
                if (tt is not None and tt.isCharOf("(,")): 
                    ...
                    if (dit is not None and dit.typ == DateItemToken.DateItemType.MONTH): 
                        dr_mon = DateReferent._new751(dt0, dit.int_value)
    ...
    def __tryAttachTimeLi(self, dts : typing.List['DateItemToken'], after_date : bool) -> 'ReferentToken':
        ...
        if (len(dts) == 1): 
            if (dts[0].typ == DateItemToken.DateItemType.HOUR and after_date): 
                time = DateReferent._new707(dts[0].int_value, 0)
                t1 = dts[0].end_token
            else: 
                return None
    ...
    def __applyDateRange(self, ad : 'AnalyzerData', its : typing.List['DateItemToken'], lang : 'MorphLang') -> 'DateRangeReferent':
        ...
        if ((its[0].can_be_year and its[1].string_value == "-" and its[2].typ == DateItemToken.DateItemType.YEAR) and (its[0].year < its[2].year)): 
            ...
            res.date_from = Utils.asObjectOrNull(ad.registerReferent(DateReferent._new704(its[0].year)), DateReferent)
            rt1 = ReferentToken(res.date_from, its[0].begin_token, its[0].end_token)
            res.date_to = Utils.asObjectOrNull(ad.registerReferent(DateReferent._new704(its[2].year)), DateReferent)
    ...
    def __applyDateRange0(self, kit : 'AnalysisKit', ad : 'AnalyzerData') -> None:
        ...
        while True:
            ...
            if (str0_ == "ON" and (isinstance(t0, TextToken))): 
                ...
                if (tok is not None): 
                    dow = DateReferent._new760(tok.termin.tag)
            ...
            if (str0_ == "С" or str0_ == "C"): 
                ...
                if ((isinstance(t.next0_, ReferentToken)) and (isinstance(t.next0_.getReferent(), DateReferent))): 
                    dr = DateRangeReferent._new706(Utils.asObjectOrNull(t.next0_.getReferent(), DateReferent))
            ...
            if (date2 is None): 
                if ((isinstance(t1, NumberToken)) and (t1).int_value is not None): 
                    nt1 = NumberHelper.tryParseNumberWithPostfix(t1)
                    if (nt1 is not None): 
                        continue
                    v = (t1).int_value
            ...
            if (year_val1 == year_val2): 
                ...
                if (DateReferent.compare(date1, date2) >= 0): 
                    continue
            if (date1 is None): 
                date1 = (Utils.asObjectOrNull(ad.registerReferent(DateReferent._new704(year_val1)), DateReferent))
                rt0 = ReferentToken(date1, t0, t0)
                kit.embedToken(rt0)
                if (t0 == t): 
                    t = (rt0)
            if (date2 is None): 
                date2 = (Utils.asObjectOrNull(ad.registerReferent(DateReferent._new704(year_val2)), DateReferent))
                rt1 = ReferentToken(date2, t1, t1)
                kit.embedToken(rt1)
                t1 = (rt1)
            t = (ReferentToken(ad.registerReferent(DateRangeReferent._new701(date1, date2)), t, t1))
