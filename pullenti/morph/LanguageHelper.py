...
class LanguageHelper:
    ...
    def getCaseAfterPreposition(prep : str) -> 'MorphCase':
        ...
        """
        wrapmc43 = RefOutArgWrapper(None)
        inoutres44 = Utils.tryGetValue(LanguageHelper.__m_prep_cases, prep, wrapmc43)
        mc = wrapmc43.value
        if (inoutres44): 
            return mc
        else: 
    ...
    def normalizePreposition(prep : str) -> str:
        wrapres45 = RefOutArgWrapper(None)
        inoutres46 = Utils.tryGetValue(LanguageHelper.__m_prep_norms, prep, wrapres45)
        res = wrapres45.value
        if (inoutres46): 
            return res
        else: 
